...
class ProductionMigrationGuard {
  ...
  constructor(args: string[] = []) {
    ...
    this.dryRun = args.includes('--dry-run') || args.includes('--dry');
  }
  ...
  async validateEnvironment(): Promise<boolean> {
    ...
    try {
      await execAsync('npx wrangler d1 info cultural-archiver --env production', {
        cwd: workersDir,
      });
    ...
    try {
      await execAsync('npx wrangler d1 migrations list cultural-archiver --env production', {
        cwd: workersDir,
      });
  ...
  async executeMigration(): Promise<boolean> {
    if (this.dryRun) {
      console.log('🧪 Dry Run Mode - Production Migration Preview');
      console.log('=============================================');
      console.log('');
      console.log('✅ This is a DRY RUN - no actual changes will be made');
      console.log('');
      console.log('The following command would be executed:');
      console.log('🔧 npx wrangler d1 migrations apply cultural-archiver --env production');
      console.log('');
      console.log('📊 Current production state would be updated based on pending migrations');
      console.log('');
      console.log('⚠️  To apply changes for real, run without --dry-run flag');
      return true;
    }
  ...
  async run(): Promise<void> {
    ...
    console.log('');
    console.log(
      `🎉 ${this.dryRun ? 'Dry run completed successfully!' : 'Production migration completed successfully!'}`
    );
    console.log('');
    ...
    if (!this.dryRun) {
      ...
      console.log('📋 To apply the migration for real:');
      console.log('   npm run migrate:prod');
      console.log('');
      console.log('   Or with automatic confirmation:');
      console.log('   npm run migrate:prod -- --yes');
    }
  }
  ...
  static showHelp(): void {
    ...
    console.log('  npm run migrate:prod -- --dry-run   # Preview changes without applying');
    console.log('');
