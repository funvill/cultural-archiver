...
class MigrationValidator {
    ...
    async validateFile(filePath: string): Promise<ValidationResult> {
        ...
        try {
            ...
            lines.forEach((line, index) => {
                ...
                if (trimmedLine.startsWith('--') && !trimmedLine.includes('TODO') && !trimmedLine.includes('FIXME')) {
                    return;
                }
    ...
    async validateMigrations(specificFile?: string): Promise<ValidationResult[]> {
        try {
            ...
            if (specificFile) {
                const filePath = specificFile.startsWith('/') ? specificFile : join(migrationsDir, specificFile);
            ...
            const results = await Promise.all(
                filesToCheck.map(file => this.validateFile(file))
            );
    ...
    formatResults(results: ValidationResult[]): void {
        ...
        results.forEach(result => {
            ...
            if (result.valid && result.warnings.length === 0) {
                ...
                console.log(`${result.valid ? '⚠️' : '❌'} ${result.file}`);
        ...
        if (totalErrors > 0) {
            ...
            console.log('See https://developers.cloudflare.com/d1/reference/sql-compatibility/ for details');
            process.exit(1);
