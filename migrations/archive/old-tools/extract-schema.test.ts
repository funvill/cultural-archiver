...
describe('SchemaExtractor', () => {
  ...
  describe('Constructor and Configuration', () => {
    ...
    it('should throw error when D1_DATABASE_ID is missing', () => {
      ...
      expect(() => new SchemaExtractor()).toThrow(
        'D1_DATABASE_ID environment variable is required'
      );
    });
  ...
  describe('Schema Extraction', () => {
      ...
      result: [
        {
          results: [
            {
              name: 'artwork',
              sql: 'CREATE TABLE artwork (id TEXT PRIMARY KEY, title TEXT NOT NULL)',
            },
      ...
      result: [
        {
          results: [
            {
              name: 'trigger_artwork_updated',
              sql: 'CREATE TRIGGER trigger_artwork_updated AFTER UPDATE ON artwork',
            },
          ],
        },
      ],
    ...
    it('should handle empty database', async () => {
      ...
      await expect(extractor.extractSchema()).rejects.toThrow(
        'Schema validation failed: No CREATE TABLE statements found'
      );
    });
  ...
  describe('SQL Generation', () => {
    it('should generate properly formatted SQL', async () => {
      const mockResponse = {
        ...
        result: [
          {
            results: [{ name: 'users', sql: 'CREATE TABLE users (id TEXT PRIMARY KEY)' }],
          },
        ],
      };
      const emptyResponse = {
        ...
        result: [
          {
            results: [
              ...
              {
                name: 'logbook',
                sql: 'CREATE TABLE logbook (id TEXT PRIMARY KEY, artwork_id TEXT)',
              },
            ],
          },
        ],
            ...
            results: [
              {
                ...
                sql: 'CREATE INDEX idx_artwork_title ON artwork(title)',
              },
            ],
          },
        ],
