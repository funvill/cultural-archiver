...
class SchemaExtractor {
  ...
  public async extractSchema(outputFile?: string): Promise<string> {
    ...
    try {
      ...
      console.info(
        `  - Sample data: ${sampleData && sampleData.trim() ? '✅ Loaded' : '❌ Not available'}`
      );
  ...
  public showHelp(): void {
    ...
    console.info(
      '  npx tsx migrations/tools/extract-schema.ts  # Defaults to 001_consolidated_baseline.sql'
    );
    console.info('  npx tsx migrations/tools/extract-schema.ts custom-schema.sql');
    console.info('');
    console.info('Options:');
    console.info(
      '  output-file    Output file path (relative to migrations/, defaults to 001_consolidated_baseline.sql)'
    );
    ...
    console.info(
      '  npx tsx migrations/tools/extract-schema.ts  # Updates 001_consolidated_baseline.sql'
    );
