...
1.2. The app bar must include navigation buttons: (+) Add, Login/Logout, About (i), Help (?), Profile  
...
3.8. Users must be able to add an optional note (≤1000 characters, completely optional)  
...
4.1. The page must display a horizontal scrolling photo carousel at the top showing newest photos first  
...
7.1. The system must handle network errors with clear error messages 7.2. The system must validate form inputs and show appropriate validation errors  
...
- **Photo Handling**: Implement client-side photo resizing to 800px before upload
...
- **File Uploads**: Handle photo uploads with progress indicators and error handling
...
2. Do we need specific error tracking integration (e.g., Sentry) for production monitoring?
