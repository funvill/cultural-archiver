 ...
 */
export async function submitArtworkEdit(
  ...
  try {
    ...
    // Validate each edit
    const allowedFields = ['title', 'description', 'artist_names', 'year_created', 'medium', 'dimensions', 'tags'];
...
export async function getPendingEdits(
  ...
  try {
    ...
    throw new ApiError(
      'Failed to retrieve pending edits',
      'PENDING_EDITS_FAILED',
      500,
      { details: { error: error instanceof Error ? error.message : 'Unknown error' } }
    );
  }
...
export async function getArtworkDetails(
  ...
  try {
    ...
    throw new ApiError(
      'Failed to retrieve artwork details',
      'ARTWORK_DETAILS_FAILED',
      500,
      { details: { error: error instanceof Error ? error.message : 'Unknown error' } }
    );
  }
 ...
 */
export async function exportArtworkOSM(
  c: Context<{ Bindings: WorkerEnv }>
): Promise<Response> {
  const artworkId = c.req.param('id');
...
${Object.entries(osmData.node.tags).map(([key, value]) => 
  `    <tag k="${key}" v="${value}"/>`
).join('\n')}
  </node>
</osm>`;
    ...
    throw new ApiError(
      'Failed to export artwork to OSM format',
      'OSM_EXPORT_FAILED',
      500,
      { details: { error: error instanceof Error ? error.message : 'Unknown error' } }
    );
