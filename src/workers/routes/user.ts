...
function validateUserPreferences(preferences: any): Record<string, unknown> {
  ...
  // Preferred radius
  if (typeof preferences.preferred_radius === 'number' && 
...
export async function exportUserData(
  ...
  try {
    ...
    c.res.headers.set('Content-Disposition', `attachment; filename="cultural-archiver-export-${userToken.substring(0, 8)}.json"`);
