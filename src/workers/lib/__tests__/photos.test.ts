...
describe('Photo Processing Utilities', () => {
  ...
  describe('File Validation', () => {
    ...
    it('should reject unsupported file types', () => {
      ...
      expect(result.errors).toContain(
        'File type image/gif is not supported. Supported types: image/jpeg, image/jpg, image/png, image/webp, image/heic, image/heif'
      );
    });
    ...
    it('should reject too many files', () => {
      const files = Array.from(
        { length: 4 },
        (_, i) => new MockFile(`content${i}`, `photo${i}.jpg`, { type: 'image/jpeg' })
      );
  ...
  describe('Error Handling', () => {
    it('should handle invalid file types consistently', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain(
        'File type image/bmp is not supported. Supported types: image/jpeg, image/jpg, image/png, image/webp, image/heic, image/heif'
      );
    });
