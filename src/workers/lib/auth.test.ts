...
describe('Authentication Core Logic', () => {
  ...
  describe('UUID Generation and Validation', () => {
    ...
    it('should validate UUID format correctly', () => {
      ...
      expect(validateUUID('123e4567-e89b-12d3-a456')).toEqual({ isValid: false, error: 'Invalid UUID format' });
    });
    ...
    it('should throw error after max retries on collision', async () => {
      ...
      await expect(generateUniqueUUID(mockEnv)).rejects.toThrow('Failed to generate unique UUID after maximum retries');
    });
  });
  ...
  describe('User Creation and UUID Claiming', () => {
    ...
    it('should reject invalid email format', async () => {
      ...
      await expect(createUserWithUUIDClaim(mockEnv, invalidEmail, uuid))
    ...
    it('should reject invalid UUID format', async () => {
      ...
      await expect(createUserWithUUIDClaim(mockEnv, email, invalidUUID))
    ...
    it('should reject duplicate email', async () => {
      ...
      await expect(createUserWithUUIDClaim(mockEnv, email, uuid))
    ...
    it('should reject already claimed UUID', async () => {
      ...
      await expect(createUserWithUUIDClaim(mockEnv, email, uuid))
        .rejects.toThrow('UUID is already claimed by another account');
    });
