...
export const DEFAULT_SIMILARITY_CONFIG: SimilarityConfig = {
  thresholds: {
    ...
    high: 0.80, // Require explicit confirmation at 80% similarity
  },
...
export function createSimilarityConfig(): SimilarityConfig {
  ...
  const config: SimilarityConfig = {
    thresholds: {
      warn: parseFloat(process.env.SIMILARITY_THRESHOLD_WARN || '') || DEFAULT_SIMILARITY_CONFIG.thresholds.warn,
      high: parseFloat(process.env.SIMILARITY_THRESHOLD_HIGH || '') || DEFAULT_SIMILARITY_CONFIG.thresholds.high,
    },
    
    weights: {
      distance: parseFloat(process.env.SIMILARITY_WEIGHT_DISTANCE || '') || DEFAULT_SIMILARITY_CONFIG.weights.distance,
      title: parseFloat(process.env.SIMILARITY_WEIGHT_TITLE || '') || DEFAULT_SIMILARITY_CONFIG.weights.title,
      tags: parseFloat(process.env.SIMILARITY_WEIGHT_TAGS || '') || DEFAULT_SIMILARITY_CONFIG.weights.tags,
    },
    
    distance: {
      maxDistanceMeters: parseInt(process.env.SIMILARITY_MAX_DISTANCE_METERS || '') || DEFAULT_SIMILARITY_CONFIG.distance.maxDistanceMeters,
      optimalDistanceMeters: parseInt(process.env.SIMILARITY_OPTIMAL_DISTANCE_METERS || '') || DEFAULT_SIMILARITY_CONFIG.distance.optimalDistanceMeters,
    },
    
    title: {
      minTitleLength: parseInt(process.env.SIMILARITY_MIN_TITLE_LENGTH || '') || DEFAULT_SIMILARITY_CONFIG.title.minTitleLength,
...
function validateSimilarityConfig(config: SimilarityConfig): void {
  ...
  if (config.thresholds.high <= config.thresholds.warn) {
    throw new Error(`High threshold (${config.thresholds.high}) must be greater than warn threshold (${config.thresholds.warn}).`);
  }
  ...
  if (config.distance.optimalDistanceMeters >= config.distance.maxDistanceMeters) {
    throw new Error(`Optimal distance (${config.distance.optimalDistanceMeters}m) must be less than max distance (${config.distance.maxDistanceMeters}m)`);
  }
