...
export class MassImportSimilarityStrategy {
  // Canonical PRD values used by unit tests
  ...
  calculateSimilarity(
    ...
    if (query.artist) {
      // Prefer candidate.created_by when present (tests use this field for artist name).
      let candidateArtist: string | null = candidate.created_by || null;

      // Fallback to parsing candidate.tags for an artist property
      if (!candidateArtist && candidate.tags) {
        try {
