...
const artworksAsSearchResults = computed(() => {
  return artworks.value.map(
    (artwork): SearchResult => ({
      ...
      title:
        typeof artwork.title === 'string' && artwork.title.trim().length > 0
          ? artwork.title.trim()
          : artwork.tags_parsed &&
              typeof (artwork.tags_parsed as any).title === 'string' &&
              (artwork.tags_parsed as any).title.trim().length > 0
            ? (artwork.tags_parsed as any).title.trim()
            : artwork.tags_parsed &&
                typeof (artwork.tags_parsed as any).name === 'string' &&
                (artwork.tags_parsed as any).name.trim().length > 0
              ? (artwork.tags_parsed as any).name.trim()
              : null,
      artist_name:
        typeof artwork.artist_name === 'string' && artwork.artist_name.trim().length > 0
          ? artwork.artist_name.trim()
          : artwork.tags_parsed &&
              typeof (artwork.tags_parsed as any).artist_name === 'string' &&
              (artwork.tags_parsed as any).artist_name.trim().length > 0
            ? (artwork.tags_parsed as any).artist_name.trim()
            : artwork.tags_parsed &&
                typeof (artwork.tags_parsed as any).artist === 'string' &&
                (artwork.tags_parsed as any).artist.trim().length > 0
              ? (artwork.tags_parsed as any).artist.trim()
              : artwork.tags_parsed &&
                  typeof (artwork.tags_parsed as any).created_by === 'string' &&
                  (artwork.tags_parsed as any).created_by.trim().length > 0
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Browse all approved artworks in our collection</p>
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="error" class="text-center py-12">
        <div class="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
          <h3 class="text-lg font-medium text-red-800 mb-2">Failed to Load Artworks</h3>
      ...
      <div v-else-if="isEmptyState" class="text-center py-12">
        <div class="max-w-md mx-auto">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">No artwork found</h3>
          <p class="text-gray-500 mb-4">Be the first to submit something!</p>
          <router-link
            to="/add"
      ...
      <div v-else>
        ...
        <div class="mb-6">
          <p class="text-sm text-gray-600">
            <span v-if="!loading">
              {{ filteredTotal }} of {{ totalItems.toLocaleString() }} artwork{{
                totalItems === 1 ? '' : 's'
              }}
              shown
            </span>
