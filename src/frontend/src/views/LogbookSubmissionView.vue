...
import { PhotoIcon, ExclamationTriangleIcon, CheckCircleIcon } from '@heroicons/vue/24/outline';
      ...
      <div data-testid="submission-header" class="bg-green-600 text-white px-6 py-4 rounded-t-lg">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p data-testid="log-visit-banner" class="text-green-100 mt-1">
              Document your visit to this artwork
            </p>
          </div>
          <button @click="goBack" class="text-green-100 hover:text-white transition-colors">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-6">
            ...
            <div data-testid="photo-upload-section">
              ...
              <p class="text-sm text-gray-600 mb-3">
                <span v-if="!isFromFastUpload"
                  >Upload a photo showing the artwork as proof of your visit</span
                >
                <span v-else
                  >Photo from your fast upload session has been automatically selected</span
                >
              </p>
                  ...
                  <div class="flex items-start">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-green-700 mt-1">
                        Photo "{{ fastUploadPhotoName }}" from your fast upload session is ready for
                        submission.
                      </p>
                    </div>
                  </div>
            ...
            <div data-testid="condition-section">
              ...
              <div class="grid grid-cols-2 gap-3">
                <label
                  ...
                  :class="{ 'border-blue-500 bg-blue-50': store.condition === 'Good' }"
                >
                  <input type="radio" v-model="store.condition" value="Good" class="sr-only" />
                  <div
                    class="w-4 h-4 border border-gray-300 rounded-full mr-3 flex items-center justify-center"
                    :class="{ 'border-blue-500': store.condition === 'Good' }"
                  >
                    <div
                      v-if="store.condition === 'Good'"
                ...
                <label
                  class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50"
                  :class="{ 'border-blue-500 bg-blue-50': store.condition === 'Damaged' }"
                >
                  <input type="radio" v-model="store.condition" value="Damaged" class="sr-only" />
                  <div
                    class="w-4 h-4 border border-gray-300 rounded-full mr-3 flex items-center justify-center"
                    :class="{ 'border-blue-500': store.condition === 'Damaged' }"
                  >
                    <div
                      v-if="store.condition === 'Damaged'"
                      class="w-2 h-2 bg-blue-500 rounded-full"
                    ></div>
                  </div>
                ...
                <label
                  class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50"
                  :class="{ 'border-blue-500 bg-blue-50': store.condition === 'Missing' }"
                >
                  <input type="radio" v-model="store.condition" value="Missing" class="sr-only" />
                  <div
                    class="w-4 h-4 border border-gray-300 rounded-full mr-3 flex items-center justify-center"
                    :class="{ 'border-blue-500': store.condition === 'Missing' }"
                  >
                    <div
                      v-if="store.condition === 'Missing'"
                      class="w-2 h-2 bg-blue-500 rounded-full"
                    ></div>
                  </div>
                ...
                <label
                  class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50"
                  :class="{ 'border-blue-500 bg-blue-50': store.condition === 'Removed' }"
                >
                  <input type="radio" v-model="store.condition" value="Removed" class="sr-only" />
                  <div
                    class="w-4 h-4 border border-gray-300 rounded-full mr-3 flex items-center justify-center"
                    :class="{ 'border-blue-500': store.condition === 'Removed' }"
                  >
                    <div
                      v-if="store.condition === 'Removed'"
                      class="w-2 h-2 bg-blue-500 rounded-full"
                    ></div>
                  </div>
