...
function removePhoto(photoId: string) {
  ...
  const hasExifLocation = selectedFiles.value.some(
    (photo: PhotoFile) => photo.exifData?.latitude && photo.exifData?.longitude
  );
...
function proceedToSearch() {
  ...
  const meta = metaWithPreview.map((p: (typeof metaWithPreview)[number]) => ({
    id: p.id,
    name: p.name,
    exifLat: p.exifLat,
    exifLon: p.exifLon,
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        >
          ...
          <p class="text-sm text-gray-500">Supports JPG, PNG, WebP up to 10MB each</p>
        ...
        <div v-if="selectedFiles.length > 0" class="mt-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <div v-for="photo in selectedFiles" :key="photo.id" class="relative group">
              <img
                :src="photo.preview"
                :alt="photo.name"
              ...
              <button
                ...
                class="absolute top-1 right-1 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"
