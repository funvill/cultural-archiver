...
describe('ArtworkDetailView', () => {
  ...
  beforeEach(async (): Promise<void> => {
    ...
    const artworkByIdFunction = vi.fn((id: string) =>
      id === '12345678-1234-5678-9abc-123456789abc' ? mockArtwork : null
    );
  ...
  describe('Data Loading', (): void => {
    it('component initializes successfully with store integration', async (): Promise<void> => {
      ...
      expect(
        freshWrapper.find('[data-testid="artwork-detail"]').exists() ||
          freshWrapper.find('h1').exists()
      ).toBe(true);
    });
  ...
  describe('Accessibility', (): void => {
    ...
    it('handles image accessibility', (): void => {
      ...
      expect(images.length === 0 || images.every(img => img.attributes('alt') !== undefined)).toBe(
        true
      );
    });
  });
  ...
  describe('Edit Mode Functionality', (): void => {
    ...
    it('shows edit form fields in edit mode', async (): Promise<void> => {
      ...
      expect(titleInput.exists() || creatorsInput.exists() || descriptionTextarea.exists()).toBe(
        true
      );
    });
    ...
    it('validates character limits for text fields', async (): Promise<void> => {
      ...
      const charCounter = charCounters.find(
        c => c.text().includes('600') || c.text().includes('512')
      );
