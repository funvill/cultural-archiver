...
vi.mock('../../components/PhotoUpload.vue', () => ({
  default: {
    name: 'PhotoUpload',
    template: '<div data-testid="photo-upload"><button @click="$emit(\'uploadSuccess\', mockData)">Upload Photos</button></div>',
    props: ['userToken'],
...
describe('SubmitView', () => {
  ...
  describe('Success Notifications', (): void => {
    it('shows success notification when upload completes', async (): Promise<void> => {
      ...
      expect(successAlert.text()).toContain('Your artwork submission has been received and will be reviewed shortly.')
