...
describe('PermissionManager', () => {
  ...
  describe('Component Initialization', () => {
    it('should render permission manager interface', async () => {
      ...
      expect(
        wrapper.find('[data-testid="search"]').exists() ||
          wrapper.find('input[placeholder*="Search"]').exists()
      ).toBe(true);
    });
  ...
  describe('Revoke Permission Dialog', () => {
    ...
    it('should open revoke permission dialog', async () => {
      const revokeButton = wrapper
        .findAll('button')
        .filter((btn: any) => btn.text() === 'Revoke')[0];
  ...
  describe('Error Handling', () => {
    ...
    it('should handle grant permission errors', async () => {
      ...
      const grantButton = wrapper
        .findAll('button')
        .filter((btn: any) => btn.text().includes('Grant Permission'))[0];
      await grantButton.trigger('click');
