...
vi.mock('../ConsentForm.vue', () => ({
  default: {
    name: 'ConsentForm',
    template: '<div data-testid="consent-form"><button @click="$emit(\'consent-given\')">Give Consent</button></div>',
    emits: ['consent-given'],
  },
}))
...
describe('PhotoUpload', () => {
  ...
  describe('Basic Rendering', (): void => {
    ...
    it('has upload interface', (): void => {
      expect(wrapper.find('[data-testid="consent-form"]').exists() || 
             wrapper.find('input[type="file"]').exists()).toBe(true)
    })
