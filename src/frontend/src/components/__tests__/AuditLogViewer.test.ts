...
describe('AuditLogViewer', () => {
  ...
  describe('Filtering Interface', () => {
    ...
    it('should reset all filters', async () => {
      ...
      const resetButton = wrapper
        .findAll('button')
        .filter((btn: any) => btn.text() === 'Reset Filters')[0];
  ...
  describe('Audit Logs Display', () => {
    ...
    it('should format dates correctly', () => {
      ...
      const hasFormattedDate = tableCells.some(
        (cell: any) => cell.text().includes('2025') && cell.text().includes(':')
      );
    ...
    it('should display action colors correctly', () => {
      ...
      const actionBadges = wrapper
        .findAll('span')
        .filter((span: any) => span.text() === 'approved' || span.text() === 'grant_permission');
  ...
  describe('Log Details Modal', () => {
    ...
    it('should open details modal when view details is clicked', async () => {
      const viewDetailsButton = wrapper
        .findAll('button')
        .filter((btn: any) => btn.text() === 'View Details')[0];
    ...
    it('should close modal when close button is clicked', async () => {
      ...
      // Close modal
      const closeButton = wrapper.findAll('button').filter(
        (btn: any) => btn.find('svg').exists() // Close X button
      )[0];
  ...
  describe('Pagination', () => {
    ...
    it('should disable previous button on first page', () => {
      const previousButton = wrapper
        .findAll('button')
        .filter((btn: any) => btn.text() === 'Previous')[0];
    ...
    it('should navigate to next page when available', async () => {
      ...
      const nextButton = wrapper.findAll('button').filter((btn: any) => btn.text() === 'Next')[0];
