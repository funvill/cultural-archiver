...
async function fetchResults() {
  ...
  const res = await fetch(createApiUrl(`/artists/search?q=${q}&limit=${limit}`));
    ...
    // API returns {success, data: {results: [...], count}, timestamp}
...
<template>
  <div class="artist-lookup">
    ...
    <div class="relative mt-2">
      ...
      <ul v-if="showDropdown && results.length" class="absolute z-10 w-full bg-white border mt-1 max-h-56 overflow-auto shadow-lg">
        <li
      ...
      <div v-else-if="showDropdown && query.trim().length > 0 && !results.length" class="absolute z-10 w-full bg-white border mt-1 px-3 py-2 text-sm text-gray-600 shadow-lg">
