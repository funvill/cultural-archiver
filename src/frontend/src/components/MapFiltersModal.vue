...
const emit = defineEmits(['update:isOpen', 'filtersChanged', 'clearListCaches', 'resetCacheTelemetry']);
...
// (Cluster toggle removed) -- clustering is now controlled globally via map filters and WebGL rendering.
...
// cluster toggle removed: clustering is handled by global filters and WebGL rendering
...
function handleToggleClick(event: Event, handler: Function, ...args: unknown[]) {
  // Stop propagation to prevent double-triggering
  event.stopPropagation();
  try {
    // Call handler with the provided args. Use a broad Function type for template compatibility.
    (handler as (...a: unknown[]) => void)(...args);
  } catch (err) {
    // Swallow errors from handler to avoid breaking UI interactions in modal
    // Handlers should manage their own errors.
    // eslint-disable-next-line no-console
    console.error('Error calling toggle handler', err);
  }
}
...
<template>
  <!-- Modal Backdrop -->
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-hidden bg-black bg-opacity-50 flex items-center justify-center"
    @click="closeModal">
        ...
        <div class="px-6 py-6 space-y-8">
          ...
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div class="text-xs text-blue-800 space-y-2">
              <p><strong>List Filters (OR logic):</strong> When multiple list filters are active, artworks from ANY
                selected list will be shown.</p>
              <p><strong>"Not Seen by Me" (Subtractive):</strong> This filter removes visited artworks from the result
                set and is always applied last.</p>
              <p><strong>Visual Priority:</strong> Markers show status - gray for visited, gold for "Want to See",
                colored for type-based defaults.</p>
            </div>
          </div>
              ...
              <div class="flex items-start space-x-3 p-4 bg-gray-50 rounded-lg">
                <label class="relative inline-flex items-center cursor-pointer">
                  <input type="checkbox" :checked="mapFilters.filtersState.showArtworksWithoutPhotos"
                    @change="mapFilters.toggleShowArtworksWithoutPhotos()" class="sr-only peer" />
                  <div
          ...
          <div v-if="!isAuthenticated" class="text-center py-8">
            <div class="bg-amber-50 border border-amber-200 rounded-lg p-6">
              ...
              <p class="text-sm text-amber-800 mb-4">Unlock powerful filtering features like "Want to See" lists and
                custom list filtering by signing in to your account.</p>
              <p class="text-xs text-amber-700">Display options like marker clustering are available without signing in.
              </p>
            </div>
          </div>
          ...
          <div v-else class="space-y-8">
            ...
            <div>
              ...
              <div>
                ...
                <div class="space-y-4">
                  ...
                  <div
                    ...
                    @click="handleToggleWantToSee">
                    <label class="relative inline-flex items-center cursor-pointer"
                      @click="handleToggleClick($event, handleToggleWantToSee)">
                      <input type="checkbox" :checked="mapFilters.isFilterEnabled('wantToSee')" class="sr-only" />
                      <div
                    ...
                    <div class="flex-1">
                      <div class="flex items-center">
                        ...
                        <span v-if="mapFilters.isFilterEnabled('wantToSee')"
                          class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                          Active
                        </span>
                      </div>
                      <p class="text-xs mt-1 text-gray-600">
                        Display only artworks you've saved to your "Want to See" wishlist. Perfect for planning art
                        exploration routes.
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    class="flex items-start space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                    @click="handleToggleNotSeenByMe">
                    <label class="relative inline-flex items-center cursor-pointer"
                      @click="handleToggleClick($event, handleToggleNotSeenByMe)">
                      <input type="checkbox" :checked="mapFilters.isFilterEnabled('notSeenByMe')" class="sr-only" />
                      <div
                    ...
                    <div class="flex-1">
                      <div class="flex items-center">
                        ...
                        <span v-if="mapFilters.isFilterEnabled('notSeenByMe')"
                          class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                          Active
                        </span>
                      </div>
                      <p class="text-xs mt-1 text-gray-600">
                        Hide artworks you've already visited or logged. Great for discovering new locations in familiar
                        areas.
                      </p>
                    </div>
                  </div>
              ...
              <div v-if="hasUserLists">
                ...
                <div class="space-y-3">
                  <div v-for="list in mapFilters.availableUserLists.value" :key="list.id"
                    class="flex items-start space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
                    @click="handleToggleUserList(list.id)">
                    <div class="relative inline-flex items-center">
                      <input type="checkbox" :checked="mapFilters.isFilterEnabled(`list:${list.id}`)" class="sr-only"
                        :id="`toggle-${list.id}`" :aria-labelledby="`label-${list.id}`" />
                      <div
                        ...
                        :aria-label="`Toggle ${list.name} filter`" role="button"
                        :aria-pressed="mapFilters.isFilterEnabled(`list:${list.id}`)" tabindex="0">
                        <div
                          class="dot absolute top-0.5 left-0.5 w-5 h-5 rounded-full transition-transform shadow-lg border border-gray-200"
                    ...
                    <div class="flex-1">
                      <div class="flex items-center justify-between">
                        <div class="flex items-center">
                          ...
                          <span v-if="mapFilters.isFilterEnabled(`list:${list.id}`)"
                            class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            Active
                          </span>
                        </div>
                        <button @click="handleToggleClick($event, handleViewList, list.id)"
              ...
              <div v-else class="text-center py-8">
                <div class="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-6">
                  ...
                  <p class="text-xs text-gray-500 mb-4">Create custom lists to organize artworks by theme, location, or
                    any criteria that matters to you.</p>
                  <p class="text-xs text-gray-400">Visit artwork detail pages to add items to lists, or create new lists
                    from your profile.</p>
                </div>
              </div>
              ...
              <div v-if="showAdvancedFeatures">
                ...
                <div v-if="mapFilters.recentlyUsedFilters.value?.length > 0" class="mb-4">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <button v-for="recent in mapFilters.recentlyUsedFilters.value.slice(0, 3)"
                      :key="recent.filterId ?? recent.id ?? recent.name" @click="applyQuickFilter(recent.filterId)"
                ...
                <div v-if="mapFilters.filterPresets.value?.length > 0" class="mb-4">
                  ...
                  <div class="space-y-2">
                    <div v-for="preset in mapFilters.filterPresets.value" :key="preset.id"
                      ...
                      <div class="flex items-center space-x-2">
                        <button @click="mapFilters.applyPreset(preset.id)"
                          class="px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">
                          Apply
                        </button>
                        <button @click="mapFilters.deletePreset(preset.id)"
                ...
                <div class="space-y-3">
                  <div class="flex items-center space-x-2">
                    <input v-model="newPresetName" type="text" placeholder="Preset name..."
                      class="flex-1 px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500" />
                    <button @click="createNewPreset" :disabled="!newPresetName.trim()"
              ...
              <div v-if="showAdvancedFeatures && showMetrics">
                ...
                <div class="bg-gray-50 p-3 rounded-md space-y-2">
                  <div class="flex justify-between text-xs">
                    ...
                    <span class="font-medium">{{ formatDuration(mapFilters.getFilterMetrics.value.sessionDuration)
                      }}</span>
                  </div>
                  <div class="flex justify-between text-xs">
                  ...
                  <div class="flex justify-between text-xs">
                    ...
                    <span class="font-medium">{{ Math.round(mapFilters.getFilterMetrics.value.cacheHitRate * 100)
                      }}%</span>
                  </div>
                  <div class="flex justify-between text-xs">
          ...
          <div v-if="isAuthenticated" class="border-t border-gray-200 pt-4">
            <button @click="showAdvancedFeatures = !showAdvancedFeatures"
              ...
              <svg :class="{ 'rotate-180': showAdvancedFeatures }" class="w-4 h-4 transition-transform" fill="none"
            ...
            <div v-if="showAdvancedFeatures" class="mt-2 pl-4">
              <label class="flex items-center text-xs text-gray-600">
                <input v-model="showMetrics" type="checkbox" class="mr-2" />
                Show performance metrics
              </label>
              <div class="mt-3">
                <button @click="$emit('clearListCaches')"
                  class="px-3 py-2 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">
                  Clear list caches
                </button>
                <button @click="$emit('resetCacheTelemetry')"
