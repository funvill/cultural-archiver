...
const changesByCategory = computed(() => {
  ...
  tagChanges.value.forEach(change => {
    ...
    const category = tagDefinition
      ? TAG_CATEGORIES.find(cat => cat.key === tagDefinition.category)
      : null;
...
<template>
  <div class="tag-diff-display">
    ...
    <div v-if="hasChanges" class="space-y-4">
      ...
      <div v-for="(changes, categoryKey) in changesByCategory" :key="categoryKey" class="space-y-2">
        ...
        <div class="space-y-2">
          <div
            ...
            :key="change.key"
            :class="['p-3 rounded-md border-l-4', getChangeColor(change.type)]"
          >
            <div class="flex items-start space-x-2">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="change.type === 'removed' || change.type === 'modified'" class="mt-1">
                  ...
                  <span
                    class="ml-1 inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800"
                  >
                    {{ change.oldValue }}
                  </span>
                </div>
    ...
    <div v-else class="text-sm text-gray-500 italic">No tag changes</div>
  </div>
</template>
