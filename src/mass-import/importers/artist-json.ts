 ...
 * 
 * 1. Legacy Format (flat structure):
 * [
 *   {
 ...
 * ]
 *
 * 2. V3 Format (with properties):
 * [
 *   {
 *     "type": "Artist",
 *     "id": "source/artist-id",
 *     "name": "Artist Name",
 *     "description": "Artist biography text...",  // optional
 *     "properties": {
 *       "source": "https://example.com",
 *       "source_url": "https://example.com/artist/123",
 *       "biography": "Artist biography text...",  // optional
 *       "birth_date": "1950",                     // optional
 *       "death_date": "2020",                     // optional
...
/**
 * V3 Artist properties structure
 */
interface ArtistV3Properties {
  source: string;
  source_url: string;
  biography?: string;
  birth_date?: string;
...
/**
 * Artist record supporting both legacy and v3 formats
 */
interface ArtistJsonRecord {
  ...
  name: string;
  description?: string;
  properties?: ArtistV3Properties;
  
  // Legacy format fields (flat structure)
  source?: string;
  source_url?: string;
  biography?: string;
  'birth date'?: string;
...
export class ArtistJsonImporter implements ImporterPlugin {
  ...
  private mapSingleArtist(artist: ArtistJsonRecord): RawImportData {
    ...
    const isV3Format = !!artist.properties;
    
    // Extract source - can be in properties (v3) or at top level (legacy)
    const source = isV3Format 
      ? artist.properties!.source 
      : artist.source!;
    
    // Extract source_url - can be in properties (v3) or at top level (legacy)
    const sourceUrl = isV3Format
      ? artist.properties!.source_url
      : artist.source_url;
    
    // Extract biography/description - multiple possible locations
    const biography = artist.description 
      || artist.biography 
      || artist.properties?.biography;

    // Build tags from all fields
    const excludedFields = ['name', 'biography', 'description', 'title', 'type', 'id', 'properties'];
    const tags: Record<string, string> = {};

    if (isV3Format) {
      // V3 format: extract from properties
      if (artist.properties) {
        for (const [key, value] of Object.entries(artist.properties)) {
          if (key !== 'source' && key !== 'source_url' && key !== 'biography' && 
    ...
    // Use source_url as external ID if available, otherwise use id or name-based ID
  ...
  async validateData(data: unknown): Promise<ValidationResult> {
    ...
    for (let i = 0; i < samplesToCheck; i++) {
      ...
      const artistRecord = artist as ArtistJsonRecord;

      // Required fields
      if (!artistRecord.name || typeof artistRecord.name !== 'string') {
        errors.push({
      ...
      // Source can be either at top level (legacy) or in properties (v3)
      const hasLegacySource = artistRecord.source && typeof artistRecord.source === 'string';
      const hasV3Source = artistRecord.properties?.source && typeof artistRecord.properties.source === 'string';

      if (!hasLegacySource && !hasV3Source) {
        errors.push({
          field: `artists[${i}].source`,
          message: 'Artist source is required (either at top level or in properties)',
          code: 'MISSING_REQUIRED_FIELD',
          severity: 'error',
        });
      }
