...
export class OSMImporter implements ImporterPlugin {
  ...
  async mapData(sourceData: unknown, config: OSMImporterConfig): Promise<RawImportData[]> {
    ...
    // Enforce new shape: require `artists` array for artist information; reject legacy `artist` string-only records
    const invalidRecords = mappedData.filter(r => (r as any).artist && !Array.isArray((r as any).artists));
    if (invalidRecords.length > 0) {
      throw new Error(
        `OSM importer: found ${invalidRecords.length} record(s) using legacy 'artist' string. Importer now requires 'artists' array. Please convert or update the source data.`
      );
    }
  ...
  private async loadArtistLookupFromConfig(config: OSMImporterConfig): Promise<void> {
    ...
    possiblePaths.push(path.resolve('src/mass-import/scraper/output/new-west-city-artists-transformed.json')); // New Westminster artists for testing
  ...
  private shouldIncludeFeature(feature: GeoJSONFeature, config: OSMImporterConfig): boolean {
    ...
    if (!(config && config.includeUnknownFeatureTypes && artworkType)) {
      // If any of the feature's type tags match the configured include list, include it (case-insensitive)
      const hasIncludedType = featureTypes.some(type => includeListArr.includes(type.toLowerCase()));
  ...
  private async mapSingleFeature(
    ...
    // Validate coordinates - skip artworks with [0, 0] coordinates
    if (lat === 0 && lon === 0) {
      const title = this.extractTitle(props);
      throw new Error(`Skipping artwork with invalid coordinates [0, 0]: "${title || 'Unnamed'}". This usually indicates missing geocoding data.`);
    }
  ...
  // Check for artists array first (preferred modern format), then fall back to extracting artist string
  let artistsArray: string[] | undefined;
  if (Array.isArray(props.artists) && props.artists.length > 0) {
    // Use the artists array directly from properties
    artistsArray = props.artists.map(a => String(a).trim()).filter(Boolean);
  } else {
    // Fall back to legacy extraction from string fields
  ...
  const tags = this.buildTags(props, config.tagMappings || {}, config.descriptionFields || []);
    ...
    const mappedRecord: RawImportData = {
      ...
      externalId,
      tags,
      // Include artists array if we have one
      ...(artistsArray && artistsArray.length > 0 ? { artists: artistsArray } : {}),
