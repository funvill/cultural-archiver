...
export class ApiExporter implements ExporterPlugin {
        ...
        for (let i = 0; i < batchResponses.length; i++) {
          ...
          if (response.success) {
            ...
            if (this.isDuplicateResponse(response)) {
              ...
              try {
                const parsed = response.data as unknown;

                // Responses sometimes nest payload under `data` (response.data.data.results)
                let resultsObj: Record<string, unknown> | undefined = undefined;
                if (parsed && typeof parsed === 'object') {
                  const parsedRec = parsed as Record<string, unknown>;
                  if (parsedRec.data && typeof parsedRec.data === 'object') {
                    resultsObj = (parsedRec.data as Record<string, unknown>).results as Record<string, unknown> | undefined;
                  }
                  if (!resultsObj && parsedRec.results && typeof parsedRec.results === 'object') {
                    resultsObj = parsedRec.results as Record<string, unknown> | undefined;
                  }
                }

                // Prefer artworks, then artists
                let createdArtworkFirst: Record<string, unknown> | undefined;
                let createdArtistFirst: Record<string, unknown> | undefined;

                const artworks = resultsObj?.artworks as unknown;
                if (artworks && typeof artworks === 'object') {
                  const artworksRec = artworks as Record<string, unknown>;
                  const createdArr = artworksRec.created as unknown;
                  if (Array.isArray(createdArr) && createdArr.length > 0) {
                    createdArtworkFirst = createdArr[0] as Record<string, unknown>;
                  }
                }

                const artists = resultsObj?.artists as unknown;
                if (artists && typeof artists === 'object') {
                  const artistsRec = artists as Record<string, unknown>;
                  const createdArr = artistsRec.created as unknown;
                  if (Array.isArray(createdArr) && createdArr.length > 0) {
                    createdArtistFirst = createdArr[0] as Record<string, unknown>;
                  }
                }

                const base = this.config?.apiEndpoint ? String(this.config.apiEndpoint) : undefined;

                const createdArtworkId = createdArtworkFirst && typeof createdArtworkFirst.id === 'string' ? String(createdArtworkFirst.id) : undefined;
                const createdArtistId = createdArtistFirst && typeof createdArtistFirst.id === 'string' ? String(createdArtistFirst.id) : undefined;

                if (createdArtworkId) {
                  if (base) {
                    try {
                      recordDataWithUrl.createdUrl = new URL(`/api/artworks/${createdArtworkId}`, base).toString();
                    } catch {
                      recordDataWithUrl.createdUrl = `/api/artworks/${createdArtworkId}`;
                    }
                  } else {
                    recordDataWithUrl.createdUrl = `/api/artworks/${createdArtworkId}`;
                  }
                } else if (createdArtistId) {
                  if (base) {
                    try {
                      recordDataWithUrl.createdUrl = new URL(`/api/artists/${createdArtistId}`, base).toString();
                    } catch {
                      recordDataWithUrl.createdUrl = `/api/artists/${createdArtistId}`;
                    }
                  } else {
                    recordDataWithUrl.createdUrl = `/api/artists/${createdArtistId}`;
                  }
   ...
   * Supports both v2 and v3 response formats
   */
  private isDuplicateResponse(response: ApiResponse): boolean {
    ...
    if (apiVersion === 'v3') {
      return this.isV3DuplicateResponse(response);
    }
    
    return this.isV2DuplicateResponse(response);
  }

  /**
   * Check for duplicate in v3 API response format
   */
  private isV3DuplicateResponse(response: ApiResponse): boolean {
    try {
      const data = response.data as Record<string, unknown>;
      
      // v3 returns: { status: "skipped", reason: "DUPLICATE_DETECTED", duplicate: {...} }
      return data.status === 'skipped' && data.reason === 'DUPLICATE_DETECTED';
    } catch {
      return false;
    }
  }
  ...
  /**
   * Check for duplicate in v2 API response format (legacy)
   */
  private isV2DuplicateResponse(response: ApiResponse): boolean {
    try {
      const data = response.data as Record<string, unknown>;
  ...
  private extractDuplicateDetails(response: ApiResponse): Record<string, unknown> | null {
    ...
    if (apiVersion === 'v3') {
      return this.extractV3DuplicateDetails(response);
    }
    
    return this.extractV2DuplicateDetails(response);
  }

  /**
   * Extract duplicate details from v3 API response
   */
  private extractV3DuplicateDetails(response: ApiResponse): Record<string, unknown> | null {
    try {
      const data = response.data as Record<string, unknown>;
      ...
      return {
        type: data.type || 'unknown', // 'artwork' or 'artist'
        existingId: duplicate.id,
        existingTitle: duplicate.title || duplicate.name,
        confidenceScore: duplicate.score || data.score,
        reason: data.reason || 'DUPLICATE_DETECTED',
        details: duplicate,
      };
    } catch {
      return null;
  ...
  /**
   * Extract duplicate details from v2 API response (legacy)
   */
  private extractV2DuplicateDetails(response: ApiResponse): Record<string, unknown> | null {
    try {
      const data = response.data as Record<string, unknown>;
  ...
  /**
   * Detect API version from endpoint URL
   */
  ...
  private transformToApiFormat(record: RawImportData | Record<string, unknown>): unknown {
    ...
    if (apiVersion === 'v3') {
      return this.transformToV3Format(record);
    }
    
    return this.transformToV2Format(record);
  }

  /**
   * Transform record to Mass Import v3 format (GeoJSON Feature or Artist JSON)
   */
  private transformToV3Format(record: RawImportData | Record<string, unknown>): unknown {
    const recordData = record as Record<string, unknown>;
    
    // Detect if this is an artist-only record
    ...
    if (looksLikeArtist) {
      // Artist JSON format for v3
      console.log(`[API-EXPORTER] v3: Detected artist record: ${recordData.title}`);
      
      // Extract source_url from tags or use externalId if it looks like a URL
      let sourceUrl = String((tags && tags.source_url) || '');
      if (!sourceUrl && externalId && (externalId.startsWith('http://') || externalId.startsWith('https://'))) {
        sourceUrl = externalId;
      }
      
      return {
        type: 'Artist',
        id: String(recordData.externalId || recordData.id || ''), // Use externalId or fallback to id
        name: String(recordData.title || ''),
        ...
        properties: {
          ...
          tags: tags,
          photos: ((recordData.photos as unknown[]) || []).slice(0, 10), // Limit to 10 photos max
          skipDuplicateDetection: this.config?.skipDuplicateDetection ?? false,
          // If exporter is configured to auto-approve artists, request the server
          // create the artist in the 'approved' state by including status in
          // the Artist properties (v3 schema allows passthrough fields).
          ...(this.config?.autoApproveArtists ? { status: 'approved' } : {}),
        },
      };
    }

    // GeoJSON Feature format for v3
    console.log(`[API-EXPORTER] v3: Detected artwork record: ${recordData.title}`);
    
    // Extract source_url from tags
    const sourceUrl = String((tags && tags.source_url) || '');
      ...
      geometry: {
        type: 'Point',
        coordinates: [Number(recordData.lon), Number(recordData.lat)],
      },
      properties: {
        title: String(recordData.title || ''),
        description: String(recordData.description || ''),
          // Support artists array (preferred) and fallback to single artist string
          ...(Array.isArray(recordData.artists) ? { artists: recordData.artists } : {}),
          ...(recordData.artist && !recordData.artists ? { artists: [String(recordData.artist)] } : {}),
        source: String(recordData.source || ''),
        source_url: sourceUrl,
        externalId: String(recordData.externalId || ''),
        tags: tags,
  ...
  /**
   * Transform record to Mass Import v2 format (legacy batch format)
   */
  private transformToV2Format(record: RawImportData | Record<string, unknown>): unknown {
    const recordData = record as Record<string, unknown>;
