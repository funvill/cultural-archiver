...
export class PluginCLI {
  ...
  private async handleImportCommand(options: CLIOptions): Promise<void> {
    ...
    const fileLogger = new FileLogger('./logs/mass-import', 'import');
    fileLogger.start();
    ...
    try {
      ...
      // Stop logger and display log file path
      fileLogger.stop();
      console.log(chalk.cyan(`\n📝 Log file saved to: ${fileLogger.getLogFilePath()}`));
    } catch (error) {
      ...
      // Stop logger and display log file path even on error
      fileLogger.stop();
      console.log(chalk.cyan(`\n📝 Log file saved to: ${fileLogger.getLogFilePath()}`));
