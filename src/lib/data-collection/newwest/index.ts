#!/usr/bin/env node
/**
 * New Westminster Public Art Registry Data Collector
 *
 * Main entry point for scraping artwork data from the City of New Westminster website.
 *
 * Usage:
...
async function main(): Promise<void> {
  logger.info('🎨 New Westminster Public Art Registry Data Collector Starting...');
  ...
  try {
    ...
    const debugDir = path.join(__dirname, config.output.debugDirectory);
    const scraper = new WebScraper(config.scraping, debugDir);
    ...
    let currentPageHtml = await scraper.fetch(currentPageUrl);

    // Collect all artwork URLs from all pages
    logger.info('\n🔍 Step 2: Collecting artwork URLs (following Next button pagination)...');
    const allArtworkUrls: string[] = [];
    let pageNumber = 1;

    // Follow "Next" button pagination until no more pages
    while (currentPageUrl !== null) {
      logger.info(`Processing page ${pageNumber}...`);
      
      // Parse current page for artwork URLs
      const urls = parser.parseArtworkIndex(currentPageHtml, config.baseUrl);
      allArtworkUrls.push(...urls);
      logger.info(`  Found ${urls.length} artworks on page ${pageNumber} (total: ${allArtworkUrls.length})`);

      // Check for Next button
      const nextUrl = parser.extractNextPageUrl(currentPageHtml, config.baseUrl);
      
      if (nextUrl) {
        logger.info(`  Next page found: ${nextUrl}`);
        currentPageUrl = nextUrl;
        currentPageHtml = await scraper.fetch(currentPageUrl);
        pageNumber++;
      } else {
        logger.info(`  No more pages (reached last page)`);
        currentPageUrl = null;
      }
    }
    ...
    if (config.validation.expectedArtworkCount && uniqueArtworkUrls.length !== config.validation.expectedArtworkCount) {
    ...
    if (artistUrls.size > 0) {
      ...
      logger.info('\n👤 Step 4: No artist URLs found to scrape');
    }
    ...
    // Validation
    if (config.validation.expectedArtworkCount && geoJSON.features.length < config.validation.expectedArtworkCount) {
    ...
    logger.info('\n⚠️  IMPORTANT: Update parser.ts with actual HTML selectors after inspecting the website');
    process.exit(0);
  } catch (error) {
